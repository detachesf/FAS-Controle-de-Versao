...
from FASgtkui import mensagem_erro, mensagem_aviso, pergunta_sim_nao
...
def cepel2lp(arqcepel, Diretorio_Padrao):
    ...
    try:
        ...
        mensagem_erro('Erro', 'Planilha CEPEL n„o encontrada ou formato n„o suportado, utilize arquivos .xlsm ou .xlsx')
    ...
    def Titulos(sheet):
        ...
        if 'ID' in titulos:  # Verifica se foi encontrado chave "ID (SAGE)"
            while True:
                ...
                if sheet.cell(row=li,
                              column=titulos['ID']).value:  # Verifica se a c‚lula est  preenchida com algum valor
    ...
    try:
        ...
        for index_linha in range(li, pds_sh.max_row + 1):
            ID = '' if str(pds_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pds_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ...
            if str(ID).strip() and str(ID).strip()[0] != ';' and c3:
                OCR = '' if str(pds_sh.cell(row=index_linha, column=titulo_dic['OCR']).value) == 'None' else str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['OCR']).value)
                NOME = '' if str(pds_sh.cell(row=index_linha, column=titulo_dic['NOME']).value) == 'None' else str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['NOME']).value)
                TIPO = '' if str(pds_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value) == 'None' else str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value)
                ALRIN = '' if str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['ALRIN']).value).strip() == 'SIM' else 'X'
                SOE = '' if str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['SOEIN']).value).strip() == 'SIM' else 'X'
                TAC = '' if str(pds_sh.cell(row=index_linha, column=titulo_dic['TAC']).value) == 'None' else str(
                    pds_sh.cell(row=index_linha, column=titulo_dic['TAC']).value)
                pds_dic[ID] = [OCR, NOME, TIPO, ALRIN, SOE, TAC]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PDS')
    ...
    try:
        ...
        for index_linha in range(li, pdd_sh.max_row + 1):
            ID = '' if str(pdd_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pdd_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';' and c3:
                PDS = '' if str(pdd_sh.cell(row=index_linha, column=titulo_dic['PDS']).value) == 'None' else str(
                    pdd_sh.cell(row=index_linha, column=titulo_dic['PDS']).value)
                pdd_dic[PDS] = [ID]
    except:
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PDD')
    ##### Ler PDF #####
    try:
        try:
        ...
        for index_linha in range(li, pdf_sh.max_row + 1):
            ID = '' if str(pdf_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pdf_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
            ...
            if str(ID).strip() and str(ID).strip()[0] != ';':
                PNT = '' if str(pdf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value) == 'None' else str(
                    pdf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value)
                ORDEM = '' if str(pdf_sh.cell(row=index_linha, column=titulo_dic['ORDEM']).value) == 'None' else str(
                    pdf_sh.cell(row=index_linha, column=titulo_dic['ORDEM']).value)
                pdf_dic[PNT] = [ID, ORDEM]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PDF')
    ...
    try:
        ...
        for index_linha in range(li, pts_sh.max_row + 1):
            ID = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pts_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                OCR = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['OCR']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['OCR']).value)
                NOME = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['NOME']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['NOME']).value)
                TIPO = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value)
                ALRIN = '' if str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['ALRIN']).value).strip() == 'SIM' else 'X'
                LSA = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['LSA']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['LSA']).value)
                LSE = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['LSE']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['LSE']).value)
                LSU = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['LSU']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['LSU']).value)
                TAC = '' if str(pts_sh.cell(row=index_linha, column=titulo_dic['TAC']).value) == 'None' else str(
                    pts_sh.cell(row=index_linha, column=titulo_dic['TAC']).value)
                pts_dic.update({ID: [OCR, NOME, TIPO, ALRIN, LSA, LSE, LSU, TAC]})
    except:
        mensagem_aviso('Aten‡„o', 'N„o foi poss¡vel processar a planilha PTS')
    ...
    try:
        ...
        for index_linha in range(li, ptd_sh.max_row + 1):
            ID = '' if str(ptd_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                ptd_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                PTS = '' if str(ptd_sh.cell(row=index_linha, column=titulo_dic['PTS']).value) == 'None' else str(
                    ptd_sh.cell(row=index_linha, column=titulo_dic['PTS']).value)
                ptd_dic[PTS] = [ID]
    except:
        mensagem_aviso('Aten‡„o', 'N„o foi poss¡vel processar a planilha PTD')
    ...
    try:
        ...
        for index_linha in range(li, ptf_sh.max_row + 1):
            ID = '' if str(ptf_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                ptf_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                PNT = '' if str(ptf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value) == 'None' else str(
                    ptf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value)
                ptf_dic[PNT] = [ID]
    except:
        mensagem_aviso('Aten‡„o', 'N„o foi poss¡vel processar a planilha PTF')
    ...
    try:
        ...
        for index_linha in range(li, pas_sh.max_row + 1):
            ID = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pas_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                OCR = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['OCR']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['OCR']).value)
                NOME = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['NOME']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['NOME']).value)
                TIPO = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['TIPO']).value)
                ALRIN = '' if str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['ALRIN']).value).strip() == 'SIM' else 'X'
                LIU = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LIU']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LIU']).value)
                LIE = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LIE']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LIE']).value)
                LIA = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LIA']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LIA']).value)
                LSA = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LSA']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LSA']).value)
                LSE = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LSE']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LSE']).value)
                LSU = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['LSU']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['LSU']).value)
                BNDMO = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['BNDMO']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['BNDMO']).value)
                TAC = '' if str(pas_sh.cell(row=index_linha, column=titulo_dic['TAC']).value) == 'None' else str(
                    pas_sh.cell(row=index_linha, column=titulo_dic['TAC']).value)
                pas_dic[ID] = [OCR, NOME, TIPO, ALRIN, LIU, LIE, LIA, LSA, LSE, LSU, BNDMO, TAC]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PAS')
    ...
    try:
        ...
        for index_linha in range(li, pad_sh.max_row + 1):
            ID = '' if str(pad_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                pad_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                PAS = '' if str(pad_sh.cell(row=index_linha, column=titulo_dic['PAS']).value) else str(
                    pad_sh.cell(row=index_linha, column=titulo_dic['PAS']).value)
                pad_dic[PAS] = [ID]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PAD')
    ...
    try:
        ...
        for index_linha in range(li, paf_sh.max_row + 1):
            ID = '' if str(paf_sh.cell(row=index_linha, column=titulo_dic['ID']).value) else str(
                paf_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                PNT = '' if str(paf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value) else str(
                    paf_sh.cell(row=index_linha, column=titulo_dic['PNT']).value)
                paf_dic[PNT] = [ID]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha PAF')
    ...
    try:
        ...
        for index_linha in range(li, cgs_sh.max_row + 1):
            ID = '' if str(cgs_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                cgs_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                NOME = '' if str(cgs_sh.cell(row=index_linha, column=titulo_dic['NOME']).value) == 'None' else str(
                    cgs_sh.cell(row=index_linha, column=titulo_dic['NOME']).value)
                PAC = '' if str(cgs_sh.cell(row=index_linha, column=titulo_dic['PAC']).value) == 'None' else str(
                    cgs_sh.cell(row=index_linha, column=titulo_dic['PAC']).value)
                TAC = '' if str(cgs_sh.cell(row=index_linha, column=titulo_dic['TAC']).value) == 'None' else str(
                    cgs_sh.cell(row=index_linha, column=titulo_dic['TAC']).value)
                TIPOE = '' if str(cgs_sh.cell(row=index_linha, column=titulo_dic['TIPOE']).value) == 'None' else str(
                    cgs_sh.cell(row=index_linha, column=titulo_dic['TIPOE']).value)
                cgs_dic.update({ID: [NOME, PAC, TAC, TIPOE]})
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha CGS')
    ...
    try:
        ...
        for index_linha in range(li, cgf_sh.max_row + 1):
            ID = '' if str(cgf_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                cgf_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                CGS = '' if str(cgf_sh.cell(row=index_linha, column=titulo_dic['CGS']).value) == 'None' else str(
                    cgf_sh.cell(row=index_linha, column=titulo_dic['CGS']).value)
                NV2 = '' if str(cgf_sh.cell(row=index_linha, column=titulo_dic['NV2']).value) == 'None' else str(
                    cgf_sh.cell(row=index_linha, column=titulo_dic['NV2']).value)
                cgf_dic[CGS] = [ID, NV2]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha CGF')
    ...
    try:
        ...
        for index_linha in range(li, tac_sh.max_row + 1):
            ID = '' if str(tac_sh.cell(row=index_linha, column=titulo_dic['ID']).value) == 'None' else str(
                tac_sh.cell(row=index_linha, column=titulo_dic['ID']).value)
            ColProc = titulo_dic.get('', '#None')  # Pegar coluna com T¡tulo vazio com informa‡„o x/y
            if ColProc != '#None':  # Se existir coluna x/y
                ...
                c3 = True
            if str(ID).strip() and str(ID).strip()[0] != ';':
                LSC = '' if str(tac_sh.cell(row=index_linha, column=titulo_dic['LSC']).value) == 'None' else str(
                    tac_sh.cell(row=index_linha, column=titulo_dic['LSC']).value)
                tac_dic[ID] = [LSC]
    except:
        print_exc(file=stdout)
        mensagem_erro('Erro', 'N„o foi poss¡vel processar a planilha TAC')
