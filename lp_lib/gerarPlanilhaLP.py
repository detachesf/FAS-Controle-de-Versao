...
try:
    ...
    FASgtkui.mensagem_erro('Erro', 'M¢dulo XlsxWriter nÑo instalado')
...
def gerarPlanilha(nome_arquivo):
    ...
    formato_centro = arquivo.add_format({
        'align': 'left',
        'valign': 'vcenter', })
    for i in ('ID (SAGE)','OCR (SAGE)','DESCRIÄéO','TIPO','COMANDO','MEDIÄéO','TELA','LISTA DE ALARMES','SOE','OBSERVAÄéO','AGRUPAMENTO'):
